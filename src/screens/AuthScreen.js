...
export default class Auth extends React.Component {
  ...
  performLogin = (email, password, successCb, errorCb) => {
    const successCallback = (response) => {
      ...
      Alert.alert('Success!', `Successfully logged in : ${JSON.stringify(decodedToken)}`)
      
      let graphSession = JSON.stringify({
      ...
      if(graphSession !== undefined) {
        AsyncStorage.setItem('@todo-graphql:session', graphSession).then(() => {
